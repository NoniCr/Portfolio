...
`
  ...
  const displayShaderSource = `
    ...
    void main () {
        ...
        float l_mono = dot(c, vec3(0.333, 0.333, 0.333));
        vec3 mono = vec3(pow(l_mono, 0.85));
...
`
  ...
  function updateCurves(dt) {
    for (let i = activeCurves.length - 1; i >= 0; i--) {
      let curve = activeCurves[i];
      curve.progress += dt * curve.speed;
  ...
  function update() {
    ...
    // Frequent random curves with lower fluid density
    autoTimer += dt;
    if (autoTimer > nextCurveInterval && activeCurves.length < 4) {
      autoTimer = 0;
      nextCurveInterval = 0.4 + Math.random() * 0.8;
      startRandomCurve();
    }
    updateCurves(dt);
  ...
  // Canvas stays fixed covering entire viewport seamlessly while scrolling
